...
class Gejala_model extends CI_Model {
    ...
    {
        //Cek apakah ada Gejala dengan Kode sama
        $filter = $this->db->select('*')->from('tb_gejala')->where('kode_gejala', $data['kode_gejala'])->get()->num_rows();
        if ($filter < 1) {
            $insert = $this->db->insert('tb_gejala', $data);
        } else {
            // set flashdata
            $this->session->set_flashdata('gagal', 'Data Gejala gagal ditambahkan.');
            redirect(base_url('admin/gejala'), 'refresh');
        }
    }

    // get kode gejala
    public function getKode($ids)
    {
        $query = $this->db->query("SELECT kode_gejala
                FROM tb_gejala
                WHERE id_gejala IN ($ids)
        ");
